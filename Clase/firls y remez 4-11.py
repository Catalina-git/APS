...
alpha_p = 1/2 #atenuacion maxima a la wp
alpha_s = 40/2 #atenuacion minima a la ws
...
ws = (0.1, 37.5)  #banda de stop [Hz]

frecuencias = np.sort(np.concatenate(((0, fs/2), wp, ws)))
deseado = [0, 0, 1, 1, 0, 0]

cant_coef = 3001 #debe ser impar y mayorigual a 1, lo aumenté de 2001 a 3001 y cumplió la plantilla
...
desired_remez = [0, 1, 0]
weight_remez = [1, 5, 1]  # prioriza la banda pasante
fir_remez=sig.remez(numtaps=cant_coef, bands=bands_remez, desired=desired_remez, weight=weight_remez, fs=fs)
w_ls, h_ls = sig.freqz(b=fir_ls, worN=np.logspace(-2, 2, 1000), fs=fs)
w_remez, h_remez = sig.freqz(b=fir_remez, worN=np.logspace(-2, 2, 1000), fs=fs)
